export default function Investors() {
  return (
    <main className="px-8 py-24 max-w-6xl mx-auto">

      {/* PRIVATE HEADER */}
      <section className="text-center mb-24">
        <p className="text-sm uppercase tracking-[0.4em] text-[#d6b97a] mb-6">
          Private Investor Overview
        </p>

        <h1 className="text-4xl md:text-5xl font-light tracking-wide mb-10 text-[#f5e6c8]">
          Signature Vending
        </h1>

        <p className="max-w-3xl mx-auto text-lg md:text-xl text-[#e6cfa3] leading-relaxed">
          Signature Vending is building a premium unattended retail network designed for hospitality,
          workplace, and high-end environments. This private page outlines the model, the economics,
          and the long-term vision behind the business.
        </p>

        <div className="h-px w-24 bg-[#6b4a2d] mx-auto mt-12"></div>
      </section>

      {/* THE OPPORTUNITY */}
      <section className="mb-24 max-w-4xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-light tracking-wide mb-8 text-[#f5e6c8]">
          The Opportunity
        </h2>

        <p className="text-lg text-[#e6cfa3] leading-relaxed mb-6">
          Traditional vending remains poorly suited to modern premium environments. Outdated machines,
          limited product quality, and friction-heavy experiences do not align with the expectations of
          today’s hotels, offices, and residential developments.
        </p>

        <p className="text-lg text-[#e6cfa3] leading-relaxed">
          At the same time, demand for on-site convenience continues to grow. Signature Vending exists
          to bridge this gap through a design-led, data-driven approach to unattended retail.
        </p>
      </section>

      {/* THE SIGNATURE SOLUTION */}
      <section className="mb-24 max-w-4xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-light tracking-wide mb-8 text-[#f5e6c8]">
          The Signature Solution
        </h2>

        <p className="text-lg text-[#e6cfa3] leading-relaxed mb-6">
          Signature Vending deploys AI-powered smart fridges that integrate seamlessly into refined spaces.
          Customers simply tap their card, take what they need, and the system automatically charges for
          items removed.
        </p>

        <p className="text-lg text-[#e6cfa3] leading-relaxed">
          No tills. No staffing. No friction. The result is a discreet retail experience that enhances
          environments rather than detracting from them.
        </p>
      </section>

      {/* THE MACHINE MODEL */}
      <section className="mb-24 max-w-4xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-light tracking-wide mb-8 text-[#f5e6c8]">
          The Machine Model
        </h2>

        <ul className="space-y-4 text-lg text-[#e6cfa3] leading-relaxed">
          <li>• Each unit operates as a standalone, income-producing asset.</li>
          <li>• Machines are purchased only once a confirmed placement is secured.</li>
          <li>• Signature Vending manages installation, stock, monitoring, and servicing.</li>
        </ul>

        <div className="mt-8 space-y-2 text-lg text-[#e6cfa3]">
          <p>Average machine cost: <span className="text-[#f5e6c8]">~£4,000 (inc VAT)</span></p>
          <p>Conservative target: <span className="text-[#f5e6c8]">~£300 net profit per machine / month</span></p>
          <p>Upside performance: <span className="text-[#f5e6c8]">~£500 net profit per machine / month</span></p>
        </div>

        <p className="mt-8 text-lg text-[#e6cfa3] leading-relaxed">
          This positions each machine to recover capital within approximately 8–14 months, before
          generating ongoing monthly cash flow.
        </p>
      </section>

      {/* PORTFOLIO SCALE */}
      <section className="mb-24 max-w-4xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-light tracking-wide mb-8 text-[#f5e6c8]">
          Portfolio Scale
        </h2>

        <p className="text-lg text-[#e6cfa3] leading-relaxed mb-6">
          The business scales through the deployment of repeatable machine placements across hospitality,
          workplace, and multi-site environments.
        </p>

        <p className="text-lg text-[#e6cfa3] leading-relaxed">
          As the machine portfolio grows, monthly profit compounds through the addition of further
          income-producing assets.
        </p>
      </section>

      {/* CAPITAL RETURNS */}
      <section className="mb-24 max-w-4xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-light tracking-wide mb-8 text-[#f5e6c8]">
          Capital & Returns
        </h2>

        <p className="text-lg text-[#e6cfa3] leading-relaxed mb-6">
          Signature Vending is structured as a cash-flow business. As the machine portfolio matures,
          profits can be allocated between reinvestment and shareholder distributions.
        </p>

        <p className="text-lg text-[#e6cfa3] leading-relaxed">
          A long-term model of partial reinvestment alongside dividend distributions enables both
          continued growth and recurring income.
        </p>
      </section>

      {/* PRIVATE CONTACT */}
      <section className="text-center mt-32 pt-16 border-t border-[#6b4a2d]/40">
        <h2 className="text-2xl md:text-3xl font-light tracking-wide mb-6 text-[#f5e6c8]">
          Private Discussions
        </h2>

        <p className="text-lg text-[#e6cfa3] leading-relaxed mb-10 max-w-3xl mx-auto">
          Signature Vending is selectively engaging with private investors and strategic partners to
          support the acquisition and deployment of its initial machine portfolio.
        </p>

        <a
          href="mailto:partners@signature-vending.com"
          className="inline-block px-10 py-4 border border-[#d6b97a] text-sm uppercase tracking-[0.35em] text-[#f5e6c8]
                     hover:bg-[#d6b97a] hover:text-[#2a140c] transition-all duration-300"
        >
          partners@signature-vending.com
        </a>
      </section>

    </main>
  );
}
